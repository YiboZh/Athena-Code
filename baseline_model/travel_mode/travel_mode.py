...
df = pd.read_csv(DATA_DIR / "swissmetro" / "swissmetro_processed.csv")

print(df['CHOICE'].value_counts())
print("Data shape:", df.shape)
choice_map = {1: 0, 2: 1, 3: 2}  # 0=Train, 1=Metro, 2=Car
...
# ------------------------------
#  Balance the dataset by random undersampling
# ------------------------------
#  Ensure each class in 'CHOICE' has the same number of samples
min_count = df['CHOICE'].value_counts().min()
df = (
    df.groupby('CHOICE', group_keys=False)
      .apply(lambda x: x.sample(n=min_count, random_state=42))
      .reset_index(drop=True)
)
print("After balancing (undersampling):")
print(df['CHOICE'].value_counts())
print("Balanced data shape:", df.shape)
